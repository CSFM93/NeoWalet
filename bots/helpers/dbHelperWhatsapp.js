...
let dbHelper = {
    async initDB(userId){
        let db
        let files = fs.readdirSync("./users/whatsapp/")

        if (files.includes(`${userId}.json`)) {
            console.log(" json file exists ")
            db = JSON.parse(fs.readFileSync(`./users/whatsapp/${userId}.json`))
            return db
        } else {
            console.log(" json file does not exist ")
            let accounts = { accounts: [] }
            let data = JSON.stringify(accounts);
            fs.writeFileSync(`./users/whatsapp/${userId}.json`, data);
            db = JSON.parse(fs.readFileSync(`./users/whatsapp/${userId}.json`))
            return db
        }
    },
    async getAccounts(userId,cb) {
        ...
        // db = await JSON.parse(fs.readFileSync(`./users/whatsapp/${userId}.json`))
